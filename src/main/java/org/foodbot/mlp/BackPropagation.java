...
public class BackPropagation extends FeedFoward {
	...
	protected List<double[][]> bpCalc(List<double[][]> activationList, List<double[][]> beforeActivationList,
		...
		for (int i = HyperParameter.BP_NUM - 1; i >= 0; i--) {
			...
			case HyperParameter.BP_NUM - 1:
				...
				// System.out.println("deltaList 1 : "
				// +tmpDeltaList.get(0).length);
				// System.out.println("weight size
				// "+HyperParameter.weights.size());
				break;
				...
				for (int j = 0; j < batchNum.length; j++) {
					...
					for (int n = 0; n < tmpWeight.length; n++) {
						...
						for (int m = 0; m < tmpWeight[n].length; m++) {
							...
							// " " + tmpWeight[n].length);
							tmpDelta[j][m] = tmpWeight[n][m] * tmpDeltaList.get(tmpDeltaList.size() - 1)[j][m]
									* mlpFunc.ReLUGradient(beforeAct[j][m]);
						}
					}
				}
				tmpDeltaList.add(tmpDelta);
		...
		for (int i = 0; i < tmpDeltaList.size(); i++) {
			// System.out.println("deltaList "+ i + " " +
			// tmpDeltaList.get(i).length + " " +
			// tmpDeltaList.get(i)[0].length);
		}
		return tmpDeltaList;
	}
